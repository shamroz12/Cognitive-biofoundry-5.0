# app.py — Streamlit demo for Cognitive Biofoundry (PPO-powered Digital Twin)
...
st.title('Cognitive Biofoundry — Hybrid Digital Twin + PPO Reinforcement Learning Demo')
...
use_ppo = st.sidebar.checkbox('Use PPO policy (models/ppo_model.zip)', 
                              value=os.path.exists('models/ppo_model.zip'))
...
T = st.sidebar.number_input('Horizon (steps)', 10, 500, 100)
...
if st.button('Run Simulation'):
    ...
    for t in range(int(T)):
        ...
        # Hybrid correction (Neural Residual) --------------------------
    ...
    ax[1].plot(t_axis[:-1], actions, color='#0B6E79')
    ax[1].set_title('Feed Profile')
